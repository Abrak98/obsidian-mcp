...
# --- Section Size Validation Tests (TC27-TC32) ---


class TestSectionSizeValidation:
    """TC27-TC32: Section size validation."""

    def test_small_h2_section_no_warning(self, validator: Validator) -> None:
        """TC27: Small h2 section - no warning."""
        content = "## Section\n\nShort content here."
        warnings = validator.validate_section_sizes(content)
        assert warnings == []

    def test_large_h2_section_warning(self, validator: Validator) -> None:
        """TC28: Large h2 section - warning."""
        content = "## Section\n\n" + "x" * 5500
        warnings = validator.validate_section_sizes(content)
        assert len(warnings) == 1
        assert warnings[0].rule == "section-too-large"
        assert warnings[0].line == 1

    def test_multiple_h2_one_exceeds(self, validator: Validator) -> None:
        """TC29: Multiple h2 sections, one exceeds threshold."""
        content = "## Small\n\nOK\n\n## Large\n\n" + "x" * 6000 + "\n\n## Another\n\nOK"
        warnings = validator.validate_section_sizes(content)
        assert len(warnings) == 1
        assert "Large" in warnings[0].message

    def test_h3_included_in_h2_size(self, validator: Validator) -> None:
        ...
        warnings = validator.validate_section_sizes(content)
        assert len(warnings) == 1  # Parent section > 6000 chars total

    def test_no_h2_sections_no_warnings(self, validator: Validator) -> None:
        """TC31: No h2 sections - no warnings."""
        content = "# Title\n\nSome content\n\n### Subsection"
        warnings = validator.validate_section_sizes(content)
        assert warnings == []

    def test_custom_threshold(self, validator: Validator) -> None:
        """TC32: Custom threshold."""
        content = "## Section\n\n" + "x" * 500
        warnings = validator.validate_section_sizes(content, threshold=100)
        assert len(warnings) == 1
