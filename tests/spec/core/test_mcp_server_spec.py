...
def test_update_note_not_found(tools: _Tools) -> None:
    with pytest.raises(ValueError, match="not found"):
        tools.update_note("nonexistent", "content")
...
def test_read_note_body_only(vault_dir: Path) -> None:
    _write_note(vault_dir, "test", "---\ntags:\n- vc\n---\nHello")
    tools = _Tools(vault_dir)
...
def test_read_note_include_frontmatter(vault_dir: Path) -> None:
    _write_note(vault_dir, "test", "---\ntags:\n- vc\n---\nHello")
    tools = _Tools(vault_dir)

    result = tools.read_note("test", include_frontmatter=True)
...
def test_instructions_language_rules(vault_dir: Path, tools: _Tools) -> None:
    instructions = tools._instructions
    assert "Language Rules" in instructions
