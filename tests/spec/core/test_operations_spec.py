...
def test_create_normalizes_typography(vault_dir: Path, ops: Operations) -> None:
    """TC45: create() normalizes fancy unicode to ASCII."""
    ops.create("test", content="Hello \u2014 World \u2192 Next")
    content = _read_note(vault_dir, "test")
    assert content == "Hello - World -> Next"


def test_update_normalizes_typography(vault_dir: Path, ops: Operations) -> None:
    """TC46: update() normalizes fancy unicode to ASCII."""
    _write_note(vault_dir, "test", "---\ntags:\n- a\n---\nOld")
    ops.vault.refresh()
    ops.update("test", "Quote: \u201cHello\u201d \u2026")
    content = _read_note(vault_dir, "test")
    assert 'Quote: "Hello" ...' in content


def test_append_normalizes_typography(vault_dir: Path, ops: Operations) -> None:
    """TC47: append() normalizes fancy unicode to ASCII."""
    ops.create("test", content="Line1")
    ops.append("test", "\u2022 Item \u2713 Done")
    content = _read_note(vault_dir, "test")
    assert "- Item [x] Done" in content


def test_replace_normalizes_new_text(vault_dir: Path, ops: Operations) -> None:
    """TC48: replace() normalizes new_text to ASCII."""
    ops.create("test", content="Hello World")
    ops.replace("test", "World", "Arrow \u2192 Next")
    content = _read_note(vault_dir, "test")
    assert content == "Hello Arrow -> Next"


def test_insert_normalizes_typography(vault_dir: Path, ops: Operations) -> None:
    """TC49: insert() normalizes text to ASCII."""
    ops.create("test", content="## Header\nContent")
    ops.insert("test", "\u2022 Bullet \u2018quoted\u2019", after="## Header")
    content = _read_note(vault_dir, "test")
    assert "- Bullet 'quoted'" in content


def test_append_section_normalizes_typography(vault_dir: Path, ops: Operations) -> None:
    """TC50: append_section() normalizes text to ASCII."""
    _write_note(vault_dir, "test", "## Items\n- Item1\n## Other")
    ops.vault.refresh()
    ops.append_section("test", "## Items", "\u25aa New \u2717 Todo")
    content = _read_note(vault_dir, "test")
    assert "- New [ ] Todo" in content


def test_update_section_normalizes_typography(vault_dir: Path, ops: Operations) -> None:
    """TC51: update_section() normalizes content to ASCII."""
    _write_note(vault_dir, "test", "## Section\nOld content\n## Other")
    ops.vault.refresh()
    ops.update_section("test", "## Section", "New \u2013 content \u21d2 done")
    content = _read_note(vault_dir, "test")
    assert "New - content => done" in content
