...
- read_note returns body only by default. Use include_frontmatter=True for full content.
...
- Note names and headings: English only
- Body content: Russian\
"""
...
def _register_tools(
    ...
    def read_note(name: str, include_frontmatter: bool = False) -> str:
        """Read note content. Returns body only by default. Use include_frontmatter=True for full content."""
        try:
            if include_frontmatter:
                return inject_context(ops.read(name))
            note = ops.vault.get_note(name)
            return inject_context(note.body)
    ...
    def get_help() -> str:
        ...
        tools_info = [
            {
                "name": "read_note",
                "params": "name: str, include_frontmatter: bool = False",
                "description": "Read note content (body only by default)",
            },
            {
                "name": "create_note",
