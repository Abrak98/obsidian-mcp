...
Q17: `read_note` возвращает frontmatter+body вместе, `update_note` ожидает только body (preserves frontmatter automatically). Асимметрия приводит к ошибкам — Claude копирует output из read_note в input для update_note и получает двойной frontmatter. Решение?
A17: Добавить параметр `include_frontmatter: bool = False` в read_note. По умолчанию возвращать только body. Минимальное изменение, обратно совместимо.

Q18: Language rules для заметок — заголовки на английском, контент на русском. Claude не знает об этом и пишет всё на одном языке. Добавить в MCP server instructions?
A18: Да, добавить в _BASE_INSTRUCTIONS секцию Language Rules.
