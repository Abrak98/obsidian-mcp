...
Narrative-Hash: 73fbc7e3be8c2e444a4e2766a55b4e13
Status: committed
...
class ValidationWarning:
    line: int
    message: str
    rule: str  # "table-blank-line" | "unclosed-code-block" | "broken-link" | "section-too-large"
...
def validate_section_sizes(content: str, threshold: int = 5000) -> list[ValidationWarning]:
...
9. **Section size warning**: h2 sections exceeding 5000 characters return warning with `[EXTRACT]` recommendation. Only h2 (##) boundaries count, h3/h4 are included in parent h2 section.
...
def validate_section_sizes(content: str, threshold: int = SECTION_SIZE_THRESHOLD):
  lines = content.split("\n")
  warnings = []

  # Find all h2 positions
  h2_positions = []  # [(line_num, heading_text)]
  for i, line in enumerate(lines):
    if re.match(H2_HEADING_RE, line):
      h2_positions.append((i, line))

  # Calculate section sizes
  for idx, (start_line, heading) in enumerate(h2_positions):
    ...
    if section_size > threshold:
      heading_text = re.match(H2_HEADING_RE, heading).group(1)
      warnings.append(ValidationWarning(
        line=start_line + 1,  # 1-indexed
        message=f"Section '{heading_text}' is {section_size} chars (>{threshold}). Consider adding [EXTRACT] marker.",
...
- All write operations → validate_section_sizes(full_content) for large sections
...
## Section Size Validation Test Cases

### TC27: Small h2 section - no warning

```text
GIVEN: content = "## Section\n\nShort content here."
WHEN: validate_section_sizes(content)
THEN: warnings == []
```

### TC28: Large h2 section - warning

```text
GIVEN: content = "## Section\n\n" + "x" * 5500
WHEN: validate_section_sizes(content)
THEN: len(warnings) == 1
  AND warnings[0].rule == "section-too-large"
  AND warnings[0].line == 1
```

### TC29: Multiple h2 sections, one exceeds threshold

```text
GIVEN: content = "## Small\n\nOK\n\n## Large\n\n" + "x" * 6000 + "\n\n## Another\n\nOK"
WHEN: validate_section_sizes(content)
THEN: len(warnings) == 1
  AND warnings[0].message contains "Large"
```

### TC30: h3 subsections included in h2 size

```text
GIVEN: content = "## Parent\n\n### Child1\n\n" + "x" * 3000 + "\n\n### Child2\n\n" + "x" * 3000
WHEN: validate_section_sizes(content)
THEN: len(warnings) == 1  # Parent section > 6000 chars total
```

### TC31: No h2 sections - no warnings
...
### TC32: Custom threshold

```text
GIVEN: content = "## Section\n\n" + "x" * 500
WHEN: validate_section_sizes(content, threshold=100)
THEN: len(warnings) == 1
```
