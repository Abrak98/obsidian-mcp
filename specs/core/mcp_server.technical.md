...
Narrative-Hash: f35f08b1ea9eff401fbab983511c5f06
Status: committed
...
def _build_instructions() -> str:
    ...
    Includes Language Rules section: note names/headings in English, body content in Russian."""
...
@server.tool()
def read_note(name: str, include_frontmatter: bool = False) -> str:
    """Read note content.
    include_frontmatter=False (default): returns body only.
    include_frontmatter=True: returns full content (frontmatter + body).
...
49. `read_note` with `include_frontmatter=False` (default) returns only body. Uses `vault.get_note(name).body`.
50. `read_note` with `include_frontmatter=True` returns full file content (frontmatter + body). Uses `ops.read(name)`.
51. `_BASE_INSTRUCTIONS` includes "Language Rules" section: note names and headings in English, body content in Russian.
...
# read_note — body only by default
def read_note(name: str, include_frontmatter: bool = False) -> str:
    if include_frontmatter:
        return _inject_context(ops.read(name))
    note = ops.vault.get_note(name)
    return _inject_context(note.body)
...
tools_info = [{"name": "read_note", "params": "name: str, include_frontmatter: bool = False", "description": "..."}, ...]
...
### TC69: read_note default (body only)

GIVEN: vault with note "test" with frontmatter {"tags": ["vc"]} and body "Hello"
WHEN: read_note(name="test")
THEN: returns "Hello" (body only, no frontmatter)

### TC70: read_note include_frontmatter=True

GIVEN: vault with note "test" with frontmatter {"tags": ["vc"]} and body "Hello"
WHEN: read_note(name="test", include_frontmatter=True)
THEN: returns full content "---\ntags:\n- vc\n---\nHello"

### TC71: instructions contain Language Rules

GIVEN: any vault
WHEN: _build_instructions() called
THEN: result contains "Language Rules" section with "Note names and headings: English" and "Body content: Russian"
